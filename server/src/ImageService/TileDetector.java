...
public class TileDetector {
    ...
    private ArrayList<MatOfPoint> getContours(Mat image){
        ArrayList<MatOfPoint> bestResult = getContours(image, 20);
        int OPTIMAL_SIZE = 14;
        int optimalDist = Math.abs(bestResult.size() - OPTIMAL_SIZE);

        for(int i = 21; i < 256; i += 2){
            var result = getContours(image, i);
            int dist = Math.abs(result.size() - OPTIMAL_SIZE);
            System.out.println(i + ": " + result.size() + " || " + dist);
    ...
    private ArrayList<MatOfPoint> getContours(Mat image, int threshold){
        ArrayList<MatOfPoint> contours = new ArrayList();
        ArrayList<MatOfPoint> tmp = getTmpFrom(image, threshold);
...
//    private int getMean(Mat img){
//        Scalar mean = Core.mean(img);
//
//
//    }

    private ArrayList<MatOfPoint> getTmpFrom(Mat image, int threshold){
        Mat clone = new Mat();
        int dilateSize = 10;

        Imgproc.cvtColor(image, clone, Imgproc.COLOR_BGR2GRAY);
        Imgproc.blur(clone,clone,new Size(5,5));
        Imgproc.threshold(clone, clone, threshold, threshold/255.0, Imgproc.THRESH_BINARY);
